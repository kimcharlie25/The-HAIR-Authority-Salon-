        ...
        <div className="relative h-48 bg-gradient-to-br from-gray-50 to-gray-100">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-brand-red text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg animate-pulse">
                SALE
              </div>
            )}
            {item.popular && (
              <div className="bg-brand-yellow text-brand-blue text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
                ⭐ POPULAR
              </div>
            )}
          </div>
          ...
          {item.isOnDiscount && item.discountPrice && (
            <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-brand-red text-xs font-bold px-2 py-1 rounded-full shadow-lg">
            ...
            <div className="flex-shrink-0">
              {!item.available ? (
                ...
                <button
                  ...
                  className="bg-brand-red text-white px-6 py-2.5 rounded-xl hover:bg-brand-red-dark transition-all duration-200 transform hover:scale-105 font-semibold text-sm shadow-lg hover:shadow-xl"
                >
                  {item.variations?.length || item.addOns?.length ? 'Customize' : 'Add to Cart'}
                </button>
              ) : (
                <div className="flex items-center space-x-2 bg-brand-yellow/30 rounded-xl p-1 border-2 border-brand-yellow">
                  <button
                    onClick={handleDecrement}
                    className="p-2 hover:bg-brand-yellow/50 rounded-lg transition-colors duration-200 hover:scale-110"
                  >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="p-6">
              ...
              {item.variations && item.variations.length > 0 && (
                <div className="mb-6">
                  <h4 className="font-semibold text-gray-900 mb-4">Choose Package</h4>
                          ...
                          >
                            ...
                            <div className="flex items-center space-x-2">
                              {selectedAddOns.find(a => a.id === addOn.id) ? (
                                <div className="flex items-center space-x-2 bg-brand-red/10 rounded-xl p-1 border-2 border-brand-red">
                                  <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className="p-1.5 hover:bg-brand-red/20 rounded-lg transition-colors duration-200"
                                  >
                                    <Minus className="h-3 w-3 text-brand-red" />
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="p-1.5 hover:bg-brand-red/20 rounded-lg transition-colors duration-200"
                                  >
                                    <Plus className="h-3 w-3 text-brand-red" />
                                  </button>
                                </div>
                              ) : (
                                <button
                                  ...
                                  className="flex items-center space-x-1 px-4 py-2 bg-brand-red text-white rounded-xl hover:bg-brand-red-dark transition-all duration-200 text-sm font-semibold shadow-lg"
                                >
              ...
              <button
                ...
                className="w-full bg-brand-red text-white py-4 rounded-xl hover:bg-brand-red-dark transition-all duration-200 font-bold flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105"
              >
